...
deploy_group() {
    ...
    if [ -z "${BIND_TO}" ]; then
        echo "creating group: ice group create --name ${MY_GROUP_NAME} --publish ${PORT}  --desired ${GROUP_DESIRED} ${AUTO} ${IMAGE_NAME} "
        ice group create --name ${MY_GROUP_NAME} --publish ${PORT} --desired ${GROUP_DESIRED} ${AUTO} ${IMAGE_NAME}
        RESULT=$?
    else
        ...
        ice group create --name ${MY_GROUP_NAME} --bind ${BIND_TO} --publish ${PORT} --desired ${GROUP_DESIRED} ${AUTO} ${IMAGE_NAME}
        echo "creating group: ice group create --name ${MY_GROUP_NAME} --bind ${BIND_TO} --publish ${PORT} --desired ${GROUP_DESIRED} ${AUTO} ${IMAGE_NAME}"
        RESULT=$?
    fi
    if [ $RESULT -ne 0 ]; then
...
if [ -z "$AUTO_RECOVERY" ];then
	echo -e "${label_color}AUTO_RECOVERY not set, defaulting to true${no_color}"
    export AUTO=""
	...
	echo -e "Invalid value for Auto-recovery ${no_color}. The Auto-recovery value should be false or true"
fi
